    ...
    for i in range(0, 160):
        ...
        if j == 0:
            try: #if exist DC0.cal, maybe a pre-calibration for example, can be readed in this way:
                DC0_cal = open(Module_path + '/Outputfiles/Calibration_files/DC0.cal', 'r')
                content_DC0_file = DC0_cal.readlines() #reads line by line and out puts a list of each line
                data_DC0_file = content_DC0_file[i+15].split() #+15 skipping the first lines
                a0_old = float(data_DC0_file[0])
                a1_old = float(data_DC0_file[1])
            except:
                #The old values of DC0.cal file (preserved for security)
                a0_old_array = [-1566.70, -390.437, -535.551, -438.883, -481.988, -540.881, -445.376, -426.473, -472.282, -400.903, -459.905, -467.821, -407.577, -434.495, -465.170, -409.004, -425.109,
                ...
                a1_old_array = [0.538861, 0.499220, 0.766733, 0.854336, 0.887407, 0.898585, 0.895778, 0.897530, 0.896703, 0.894912, 0.899244, 0.899670, 0.894926, 0.890571, 0.893464, 0.885255, 0.864081,
                ...
                a0_old = a0_old_array[i]
                a1_old = a1_old_array[i]

        #DC1
        elif j == 1:
            try: #if exist DC1.cal, maybe a pre-calibration for example, can be readed in this way:
                DC1_cal = open(Module_path + '/Outputfiles/Calibration_files/DC1.cal', 'r')
                content_DC1_file = DC1_cal.readlines() #reads line by line and out puts a list of each line
                data_DC1_file = content_DC1_file[i+15].split() #+15 skipping the first lines
                a0_old = float(data_DC1_file[0])
                a1_old = float(data_DC1_file[1])
            except:
                #The old values of DC1.cal file (preserved for security)
                a0_old_array = [0.00000, -112.520, -316.765, -348.449, -121.201, -383.663, -407.282, -374.023, -435.991, -380.526, -449.580, -441.438, -400.905, -335.731, -387.642, -376.136, -404.728,
                ...
                a1_old_array = [1.00000, 0.203792, 0.588909, 0.630448, 0.760688, 0.689855, 0.784863, 0.764139, 0.803497, 0.796596, 0.805102, 0.797530, 0.804674, 0.792363, 0.796973, 0.791762, 0.782534,
        ...
        #DC2
            ...
            try: #if exist DC1.cal, maybe a pre-calibration for example, can be readed in this way:
                DC2_cal = open(Module_path + '/Outputfiles/Calibration_files/DC2.cal', 'r')
                content_DC2_file = DC2_cal.readlines() #reads line by line and out puts a list of each line
                data_DC2_file = content_DC2_file[i+15].split() #+15 skipping the first lines
                a0_old = float(data_DC2_file[0])
                a1_old = float(data_DC2_file[1])
            except:
                #The old values of DC2.cal file (preserved for security)
                a0_old_array = [-1579.39, -172.692, -206.789, -185.754, -233.720, -220.204, -253.758, -264.335, -180.604, -164.953, -252.854, -228.248, -239.096, -200.934, -237.996, -174.136, -155.608,
                ...
                a1_old_array = [0.538573, 0.443862, 0.725547, 0.814475, 0.838539, 0.850956, 0.858326, 0.858653, 0.855776, 0.855114, 0.856457, 0.850111, 0.848770, 0.844695, 0.849079, 0.847980, 0.831972,
        ...
        #DC3
            ...
            try: #if exist DC1.cal, maybe a pre-calibration for example, can be readed in this way:
                DC3_cal = open(Module_path + '/Outputfiles/Calibration_files/DC3.cal', 'r')
                content_DC3_file = DC3_cal.readlines() #reads line by line and out puts a list of each line
                data_DC3_file = content_DC3_file[i+15].split() #+15 skipping the first lines
                a0_old = float(data_DC3_file[0])
                a1_old = float(data_DC3_file[1])
            except:
                #The old values of DC3.cal file (preserved for security)
                a0_old_array = [0.00000, -148.240, -327.519, -274.980, -562.887, -489.022, -435.075, -463.304, -551.986, -474.507, -529.363, -450.051, -464.649, -445.515, -507.015, -403.313, -443.418,
                ...
                a1_old_array = [1.00000, 0.223700, 0.622980, 0.716927, 0.837177, 0.844758, 0.854938, 0.858971, 0.862239, 0.855728, 0.859681, 0.854750, 0.857246, 0.853100, 0.855021, 0.848589, 0.839035,
